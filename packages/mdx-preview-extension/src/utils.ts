import { Theme } from '@fable-doc/common/dist/cjs/types';
  ...
  if (headingElement?.parentElement) {
    ...
    if (!document.getElementById(NEW_ELEMENT_ID)) {
      ...
      iframe.onload = () => {
        ...
        iframe.contentWindow?.postMessage({ type: Msg.MDX_DATA, data }, '*');
      };
    } else {
...
const githubBotApiCall = async () => {
  ...
  const rootCssData = createRootCssContent(data.config.theme as Theme);
