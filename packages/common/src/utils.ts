import {
  Config, FSSerNode, FSSerialized, FileDetail,
  SidepanelLinkInfoNode, Theme, UrlEntriesMap,
  UrlMap, UserUrlMapFn
} from './types';
...
const traverseConfig = (config: Config, configPath: string[]): any => {
  let obj = config;
  configPath.forEach(key => obj = obj[key]);
  return obj;
};
...
export const generateUserAndDefaultCombinedConfig = (userConfig: Config, manifest: FSSerialized, currPath: string) => {
  ...
  const combinedTheme = mergeObjects(defaultConfig.theme, userConfig.theme);
...
export const getUrlMap = (fsSerManifest: FSSerialized, userUrlMap: UrlMap | UserUrlMapFn, currPath: string): UrlMap => {
  ...
  filePaths.forEach(obj => {
    urlMap[convertFilePathToUrlPath(obj.filePath)] = {
...
export const getFilePaths = (node: FSSerNode, currPath: string): FileDetail[] => {
  ...
  const traverse = (currentNode: any, currentPath: any) => {
    if (currentNode.nodeType === 'file' && currentNode.ext === '.mdx') {
      ...
      const filePath = parseFilePath(getRelativePath(currentNode.absPath as string, currPath));
...
const convertFilePathToUrlPath = (filePath: string): string => {
  const segments = filePath.split('/');
  const lastSegment = segments[segments.length - 1];

  if (lastSegment === 'index') return segments.slice(0, -1).join('/') || '/';
  else if (lastSegment === '') return filePath.slice(0, -1) || '/';
...
export const getSidepanelLinks = (rootNode: FSSerNode, urlMap: UrlMap, currPath: string): SidepanelLinkInfoNode => {
  const rootLinksTree: SidepanelLinkInfoNode = {
    ...getFolderLinkInfo(rootNode, urlMap, currPath),
    url: urlMap.globalPrefix ? `/${urlMap.globalPrefix}` : '/'
  };
  const queue = [{ fsserNode: rootNode, linksTree: rootLinksTree }];
...
const getFolderLinkInfo = (node: FSSerNode, urlMap: UrlMap, currPath: string): SidepanelLinkInfoNode => {
  ...
  if (indexFile?.frontmatter?.urlTitle) {
    info = {
...
const getPathFromFile = (filePath: string, urlMap: UrlMap, currPath: string): string => {
  const relPath = parseFilePath(getRelativePath(filePath, currPath));
