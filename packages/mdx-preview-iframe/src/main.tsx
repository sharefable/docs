...
import {
  hambergerSvg, hamburgerCode, hamburgerCss,
  headerCss, headerCode, indexCss, layoutCode,
  sidePanelCode, sidePanelCss
} from './static-files';
...
const getBuild = async (entryPoint: string, buildType: 'react' | 'mdx') => {
  try {
    ...
    if (buildType === 'mdx') {
      ...
      await getBuild(FileName.INDEX_JSX, 'react');
    } else {
...
const init = async (code: string) => {
  ...
  if (input[FileName.CONFIG_JSON] && input[FileName.MANIFEST_JSON]) { await getBuild(FileName.CODE_MDX, 'mdx'); }
};
...
const Container = () => {
  async function handleMessage (event: MessageEvent) {
    if (event.data.type === Msg.MDX_DATA) {
      ...
      await init(event.data.data as string);
