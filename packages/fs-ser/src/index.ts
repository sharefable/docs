...
import {
  contentReaderVisitor,
  cleanupVisitor, contentTransformerVisitor, contentGeneratorVisitor } from './visitors.js';
...
export interface FSSerializationOptions {
  ...
  /**
    * List of all dirs that are not be traversed while serializing the FS.
    * By default `**\/node_modules\/**` and `**\/.git\/**` are not
  ...
  /**
    ...
    * Since this is file system serialization this visitor object can't create or delete new node
    * (we don't write to fs during this phase).
...
/**
  ...
  * recursively. FS is serialized as object of type {@link FSSerNode} for each file/dir
  * that is encountered during traversal.
  * The final content is either written to a file if `opts.outputFilePath` is passed
  * or is simply returned to the caller.
